...
function runIntro() {
  ...
  INTRO_LINES.forEach((l, i) => {
    ...
    row.innerHTML = `<span class="intro-lnum">${l.text ? i + 1 : ''}</span><span>${l.text}${i === INTRO_LINES.length - 1 ? '<span class="intro-cursor"></span>' : ''}</span>`;
  ...
  const lastDelay = INTRO_LINES[INTRO_LINES.length - 1].delay;
